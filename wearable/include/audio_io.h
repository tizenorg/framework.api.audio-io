...
/**
 * @brief Enumerations of audio source type
 */
typedef enum {
    AUDIO_IO_SUPPORT_SOURCE_TYPE_DEFAULT, /* High Gain with No filter */
    AUDIO_IO_SOURCE_TYPE_MIRRORING,
    AUDIO_IO_SOURCE_TYPE_VOICECONTROL, /* VoiceControl Gain with No filter */
    AUDIO_IO_SOURCE_TYPE_SVR, /* VoiceRecognition Gain with No filter  */
    AUDIO_IO_SOURCE_TYPE_VIDEOCALL, /* Not used for AudioIn now.... */
    AUDIO_IO_SOURCE_TYPE_VOICERECORDING /* Low Gain with Boosting filter */
} audio_io_source_type_e;
...
typedef enum
{
    ...
    AUDIO_IO_INTERRUPTED_BY_NOTIFICATION, 				/**< Interrupted by notification*/
 ...
 * @param[in]  channel	The audio channel type, mono, or stereo
 * @param[in]  type	The type of audio sample (8- or 16-bit)
 * @param[in]  source_type	 The type of audio_io_support_source_type_e.
 * @param[out] input	An audio input handle will be created, if successful
 *
 * @return 0 on success, otherwise a negative error value.
 * @retval #AUDIO_IO_ERROR_NONE Successful
 ...
 * @see audio_in_destroy()
 */
int audio_in_create_ex(int sample_rate, audio_channel_e channel, audio_sample_type_e type , audio_in_h* input, audio_io_source_type_e source_type);
