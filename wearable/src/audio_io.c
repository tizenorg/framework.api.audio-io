...
{
	return audio_in_create_ex(sample_rate, channel, type , input, AUDIO_IO_SUPPORT_SOURCE_TYPE_DEFAULT);
}
...
int audio_in_create_ex(int sample_rate, audio_channel_e channel, audio_sample_type_e type , audio_in_h* input, audio_io_source_type_e source_type)
{
	...
	ret = mm_sound_pcm_capture_open_ex( &handle->mm_handle,sample_rate, channel, type, source_type);
	if( ret < 0) {
		free (handle);
		return __convert_error_code(ret, (char*)__FUNCTION__);
	}
	LOGI("[%s] mm_sound_pcm_capture_open_ex() success",__FUNCTION__);
...
{
	AUDIO_IO_NULL_ARG_CHECK(input);
	AUDIO_IO_NULL_ARG_CHECK(latency);
	audio_in_s *handle = (audio_in_s *) input;

	int ret = mm_sound_pcm_get_latency(handle->mm_handle, latency);
	if (ret != MM_ERROR_NONE) {
		return __convert_error_code(ret, (char*)__FUNCTION__);
	}

	LOGI("[%s] latency = %d",__FUNCTION__, *latency);
	return AUDIO_IO_ERROR_NONE;
}
...
int audio_out_create(int sample_rate, audio_channel_e channel, audio_sample_type_e type, sound_type_e sound_type,  audio_out_h* output)
{
	...
	if(__check_parameter(sample_rate, channel, type)!=AUDIO_IO_ERROR_NONE)
		return AUDIO_IO_ERROR_INVALID_PARAMETER;
	if(sound_type < SOUND_TYPE_SYSTEM || sound_type > SOUND_TYPE_VOIP) {
